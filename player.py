...
class Player():
    ...
    def __init__(self, joystickID, X, Y):
        ...
        self.playerimage = pygame.image.load("textures/players/player{}/sprite_0.png".format(self.joystickID))
        ...
        self.weapon = []
    def movement(self):
        self.playerVX = 1.6 * round(self.player.get_axis(0), 2)
        self.playerVY = 1.6 * round(self.player.get_axis(1), 2)
        ...
        if self.player.get_name() == 'Controller (XBOX 360 For Windows)':
            if round(self.player.get_axis(3), 2) != 0 or round(self.player.get_axis(4), 2) != 0:
                self.playerAngle = math.atan2(round(self.player.get_axis(4), 2), round(self.player.get_axis(3), 2))

        elif self.player.get_name() == 'PC/PS3/Android':
            if round(self.player.get_axis(2), 2) != 0 or round(self.player.get_axis(3), 2) != 0:
                self.playerAngle = math.atan2(round(self.player.get_axis(2), 2), round(self.player.get_axis(3), 2))
        else:
            if round(self.player.get_axis(2), 2) != 0 or round(self.player.get_axis(3), 2) != 0:
                self.playerAngle = math.atan2(round(self.player.get_axis(3), 2), round(self.player.get_axis(2), 2))
    ...
    def interreaction(self, bullets_in_game, bullet):
        if len(self.weapon) != 0:
            if self.player.get_button(4) == 1:
                ...
                self.pop_weapon()
            elif self.weapon[0].TYPE in (1, 2) and self.player.get_button(5) == 1 and self.cooldown == 0:
                # self.effect.stop()
                self.sound_shot.play()
                self.fire(bullets_in_game, bullet)
            elif self.weapon[0].TYPE in (1, 2) and self.weapon[0].bullets <= 0:
                self.pop_weapon()
    ...
    def draw(self, screen):
        ...
        if len(self.weapon) != 0 and self.weapon[0].TYPE in (1, 2) and self.alive == 1:
            self.ammo_text = self.font_type.render(f"{self.joystickID} {self.weapon[0].bullets}/{self.weapon[0].MAX_BULLETS}", True,
                                                   (0, 0, 0))
        else:
    ...
    def fire(self, bullets_in_game, Bullet):
        ...
        bullets_in_game.append(Bullet(self.playerX,
                                      self.playerY,
                                      8 * math.sin(self.playerAngle),
                                      8 * math.cos(self.playerAngle),
                                      self.joystickID))
    ...
    def change_outfit(self):
        if (len(self.weapon) == 0):
            ...
            if (self.weapon[0].TYPE == 1):
                self.playerimage = pygame.image.load("textures/players/player{}/ak47player.png".format(self.joystickID))
                self.playerold = self.playerimage
            elif (self.weapon[0].TYPE == 2):
                self.playerimage = pygame.image.load("textures/players/player{}/pistolplayer.png".format(self.joystickID))
                self.playerold = self.playerimage
            elif (self.weapon[0].TYPE == 3):
                self.playerimage = pygame.image.load("textures/players/player{}/knifeplayer.png".format(self.joystickID))
                self.playerold = self.playerimage
            elif (self.weapon[0].TYPE == 4):
                self.playerimage = pygame.image.load("textures/players/player{}/baseballplayer.png".format(self.joystickID))
                self.playerold = self.playerimage

    def agony(self, vx_bullet, vy_bullet):
        self.alive = 0
        self.playerX += vx_bullet
        self.playerY += vy_bullet
        self.playerVX = self.playerVY = 0
        self.playerAngle = math.degrees(math.atan2(vx_bullet, vy_bullet))
        self.playerimage = pygame.image.load("textures/players/player{}/deadplayer.png".format(self.joystickID))
        self.playerold = self.playerimage
